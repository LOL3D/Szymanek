...
def mouseClicked():
    if mouseX >100 and mouseX<200:
        if mouseY >10 and mouseY <30:
            library.lendBook(Przemek.requestBook("Sens Sztuki"))
        if mouseY >40 and mouseY <60:
            library.addBook(Przemek.returnBook())
            
class TestBiblioteki(unittest.TestCase):
    
    def testuj_wypozyczanie(self): #Sprawdza czy książka została wypożyczona
        
        books = ["Naocznosc", "Sens Sztuki", "Harry Potter", "Rok 1984"]
        books_testowane = ["Naocznosc", "Harry Potter", "Rok 1984"]
        library = Library(books)
        Przemek = Customer()
        library.lendBook(Przemek.requestBook("Sens Sztuki"))
        self.assertEqual(books, books_testowane)
    
    def testuj_posiadanie(self): #Sprawdza czy klient posiada książkę
        
        books = ["Sens Sztuki", "Naocznosc", "Harry Potter", "Rok 1984"]
        library = Library(books)
        Przemek = Customer()
        library.lendBook(Przemek.requestBook("Sens Sztuki"))
        self.assertTrue(Przemek.haveBook)
